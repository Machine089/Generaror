from Gen import name_g, phone_g, hash_k
from Gen.transaction import trans_creation
...
class User:
    ...
    def __init__(self, nums):
        ...
        self.phone = phone_g
        self.hash = hash_k
        self.username = name_g.names
        self.user_surnames = name_g.surnames
    ...
    def new_user(self):
        ...
        while num >= 1:
            ...
            user_list.append({
                "user": self.user.gen_unique_names(names=self.username, surnames=self.user_surnames, how_many=num),
...
def data_entry():
    user_create = User(1000)
    count = user_create.new_user()
    for item in count:
        print("Processing...")
        c.execute(
            "INSERT INTO Users(username, phone, hash_pass, transaction_sum,"
            " transaction_direction, transaction_client, transaction_type) "
